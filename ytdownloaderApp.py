...
index.set(0)
cwd = str(Path.cwd())
img = PhotoImage(file=cwd+"\\bg.png")
f1= Frame()
bg = Label(f1,image=img)
bg1 = Label(root,image=img)

bg.pack(side="top",expand=0)
...
def Download():
    try:
        def downloadstart(index):
            path = str(Path.home())      
            yn = messagebox.askokcancel("Continue Download",message=" Continue Download" +yt_video._title)
            if yn:
        ...
        showMsg(res_list)
        Label(root, text='Select Resolution',bg='#947d43',font=("Arial", 18)).place(y='200',x='130')
